            ...
            <div class="box">
                <div class="box-header with-border">
                    ...
                    <div class="pull-right">
                        <form name="filter" id="filter" method="GET" action="{{ route('admin.state.list') }}">
                           
                            <div class="col-sm-6">
                                <label class="w-100">[State Name] 
                            ...
                            <div class="col-sm-6">
                                ...
                                <a class="btn btn-primary modal-action-btn" href="{{ route('admin.state.list') }}" style="margin-top:20px;">Reset</a>
                            ...
                            @if(count($list)>0)
                                <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                                    ...
                                    <tr role="row">
                                        ...
                                        @if ( (Auth::guard('admin')->user()->role_id==1) || (in_array('admin.state.status',$getAllRoles)))
                                        ...
                                        @if ( (Auth::guard('admin')->user()->role_id==1) || (in_array('admin.state.edit',$getAllRoles)) || (in_array('admin.state.delete',$getAllRoles)))
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending">
                                            Action
                                        </th>
                                        @endif
                                        ...
                                        <tr role="row" class="odd">
                                            ...
                                            <td>
                                                @if(isset($row->status))
                                                    ...
                                                    @if($row->status == '1')
                                                        <a onclick="return confirm('Are you sure ? Want to change the status!!')" href="{{ route('admin.state.status', [$row->id]) }}" title="Change Status" style="color:green">Active</a>
                                                    @else
                                                        <a onclick="return confirm('Are you sure ? Want to change the status!!')" href="{{ route('admin.state.status', [$row->id]) }}"  title="Change Status" style="color:red">Inactive</a>
                                                    @endif  
                                                    @else
                                                        No records found
                                                    @endif
                                            ...
                                            <td>
                                                <button>
                                                @if ( (Auth::guard('admin')->user()->role_id==1) || (in_array('admin.state.edit',$getAllRoles)))
                                                <a href="{{ route('admin.state.edit', [$row->id]) }}" title="Edit">
                                                ...
                                                &nbsp;
                                                <button>
                                                @if ( (Auth::guard('admin')->user()->role_id==1) || (in_array('admin.state.delete',$getAllRoles)))
                                                <a onclick="return confirm('Are you sure ? Want to remove')" href="{{ route('admin.state.delete',[$row->id]) }}" title="Delete">
