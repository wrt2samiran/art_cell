	...
	<section>
		<div class="container">
          ...
          @foreach($order->ordered_shared_services as $ordered_shared_service)
					<tr>
						<td class="qty">{{$ordered_shared_service->quantity}}</td>
						<td class="desc">
							<h5 class="item_title">{{$ordered_shared_service->shared_service->name}}</h5>
              ...
              @endif 
						...
						<td class="total">{{$order->order_currency}} {{number_format($ordered_shared_service->total_price, 2, '.', '')}}</td>
					</tr>
          @endforeach
          @else
